...
func main() {
	...
	// 7. 创建 HTTP 服务器
	httpServer := createHTTPServer(cfg, orderAppService)
...
func createHTTPServer(cfg *config.Config, orderAppService *application.OrderApplicationService) *http.Server {
	router := gin.Default()
