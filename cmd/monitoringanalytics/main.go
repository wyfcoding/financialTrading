...
type AppContext struct {
	Config     *Config
	AppService *application.MonitoringAnalyticsService
...
func registerGRPC(s *grpc.Server, srv any) {
	ctx := srv.(*AppContext)
	pb.RegisterMonitoringAnalyticsServiceServer(s, grpchandler.NewGRPCHandler(ctx.AppService))
}
...
func registerGin(e *gin.Engine, srv any) {
	...
	// 3. 业务路由
	httpHandler := httphandler.NewMonitoringAnalyticsHandler(ctx.AppService)
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 3. 仓储与服务装配
	bootLog.Info("initializing monitoring and analytics service...")
