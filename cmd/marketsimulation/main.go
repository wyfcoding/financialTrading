...
import (
	...
	redisrepo "github.com/wyfcoding/financialtrading/internal/marketsimulation/infrastructure/persistence/redis"
	grpcserver "github.com/wyfcoding/financialtrading/internal/marketsimulation/interfaces/grpc"
	httpserver "github.com/wyfcoding/financialtrading/internal/marketsimulation/interfaces/http"
...
var configPath = flag.String("config", "configs/marketsimulation/config.toml", "config file path")
...
func main() {
	...
	if cfg.Server.Environment == "dev" {
		if err := db.RawDB().AutoMigrate(&mysql.SimulationModel{}, &outbox.Message{}); err != nil {
	...
	// 7. Repositories
	repo := mysql.NewSimulationRepository(db.RawDB())
	readRepo := redisrepo.NewSimulationRedisRepository(redisClient)
	publisher := outbox.NewPublisher(outboxMgr)

	// 8. Application Services
	commandSvc := application.NewMarketSimulationCommandService(repo, readRepo, publisher)
	querySvc := application.NewMarketSimulationQueryService(repo, readRepo)
	if err := commandSvc.ResumeRunning(context.Background()); err != nil {
		slog.Error("failed to resume running simulations", "error", err)
	}
	...
	httpHandler.RegisterRoutes(r.Group(""))
