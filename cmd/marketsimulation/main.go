...
type AppContext struct {
	Config     *Config
	AppService *application.MarketSimulationService
...
func registerGRPC(s *grpc.Server, srv any) {
	ctx := srv.(*AppContext)
	pb.RegisterMarketSimulationServiceServer(s, grpchandler.NewGRPCHandler(ctx.AppService))
}
...
func initService(cfg any, m *metrics.Metrics) (any, func(), error) {
	...
	// 3. 业务组件装配
	bootLog.Info("initializing market simulation service...")
