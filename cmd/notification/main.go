...
func main() {
	...
	log.InfoContext(ctx, "Starting NotificationService", "version", cfg.Version)
	...
	// 自动迁移
	if err := gormDB.AutoMigrate(&repository.NotificationModel{}); err != nil {
	...
	repo := repository.NewNotificationRepository(gormDB.DB)
	...
	<-quit
	log.InfoContext(ctx, "Shutting down NotificationService")
...
func createHTTPServer(cfg *config.Config, app *application.NotificationService, rateLimiter ratelimit.RateLimiter) *http.Server {
	router := gin.Default()
