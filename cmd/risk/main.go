// RiskService 主程序
// 功能：提供风险管理服务，包括风险评估、指标计算、限额检查、告警管理
...
func main() {
	// 1. 加载配置
	configPath := "configs/risk/config.toml"
	...
	log.InfoContext(ctx, "Starting RiskService",
		"version", cfg.Version,
	...
	// 7. 初始化仓储
	assessmentRepo := repository.NewRiskAssessmentRepository(database)
	metricsRepo := repository.NewRiskMetricsRepository(database)
	limitRepo := repository.NewRiskLimitRepository(database)
	alertRepo := repository.NewRiskAlertRepository(database)

	// 8. 初始化应用服务
	riskAppService := application.NewRiskApplicationService(assessmentRepo, metricsRepo, limitRepo, alertRepo)
	...
	// 10. 创建 HTTP 服务器
	httpServer := createHTTPServer(cfg, riskAppService, rateLimiter)

	// 11. 创建 gRPC 服务器
	grpcServer := createGRPCServer(cfg, riskAppService)
	...
	log.InfoContext(ctx, "Shutting down RiskService")
	...
	log.InfoContext(ctx, "RiskService stopped")
}

// createHTTPServer 创建 HTTP 服务器
func createHTTPServer(cfg *config.Config, riskAppService *application.RiskApplicationService, rateLimiter ratelimit.RateLimiter) *http.Server {
	router := gin.Default()
	...
	// 注册路由
	httpHandler := httphandler.NewRiskHandler(riskAppService)
...
func createGRPCServer(cfg *config.Config, riskAppService *application.RiskApplicationService) *grpc.Server {
	// 创建 gRPC 服务器选项
	...
	// 注册服务
	handler := grpchandler.NewGRPCHandler(riskAppService)
	pb.RegisterRiskServiceServer(server, handler)
