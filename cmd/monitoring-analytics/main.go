...
func main() {
	...
	log.InfoContext(ctx, "Starting MonitoringAnalyticsService", "version", cfg.Version)
	...
	<-quit
	log.InfoContext(ctx, "Shutting down MonitoringAnalyticsService")
...
func createGRPCServer(cfg *config.Config, app *application.MonitoringAnalyticsService) *grpc.Server {
	...
	pb.RegisterMonitoringAnalyticsServiceServer(server, handler)
