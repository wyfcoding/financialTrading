...
import (
	...
	"github.com/fynnwu/FinancialTrading/internal/clearing/infrastructure/repository"
	"github.com/fynnwu/FinancialTrading/internal/clearing/interfaces"
...
func main() {
	...
	logger.Info(ctx, "Starting ClearingService",
	...
	logger.Info(ctx, "Shutting down ClearingService")
	...
	logger.Info(ctx, "ClearingService stopped")
}
...
func createGRPCServer(cfg *config.Config, clearingAppService *application.ClearingApplicationService) *grpc.Server {
	...
	// 注册服务
	handler := interfaces.NewGRPCHandler(clearingAppService)
