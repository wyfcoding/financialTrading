...
import (
	...
	marginlendingapp "github.com/wyfcoding/financialtrading/internal/marginlending/application"
...
func main() {
	...
	dsn := os.Getenv("MARGIN_LENDING_DSN")
	if dsn == "" {
	...
	app := marginlendingapp.NewService(repo, logger)
	svc := grpc_server.NewServer(app)

	addr := os.Getenv("MARGIN_LENDING_GRPC_ADDR")
	if addr == "" {
		addr = ":9099"
	}
	...
	go func() {
		logger.Info("marginlending service started", "addr", addr)
	...
	<-quit
	logger.Info("shutting down marginlending service...")
	s.GracefulStop()
}
