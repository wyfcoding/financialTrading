...
import (
	...
	"github.com/wyfcoding/financialTrading/internal/backtest/infrastructure"
...
func main() {
	...
	// 自动迁移
	db.AutoMigrate(&domain.BacktestTask{}, &domain.BacktestReport{})

	// 3. 依赖注入
	engine := &domain.BacktestEngine{}
	repo := infrastructure.NewBacktestRepository(db)
	appService := application.NewBacktestApplicationService(engine, repo, logger)
	handler := interfaces.NewBacktestHandler(appService)
	...
	pb.RegisterBacktestServiceServer(grpcServer, handler)
	...
	logger.Info("shutting down backtest server...")
