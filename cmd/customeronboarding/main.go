...
import (
	...
	"github.com/wyfcoding/financialtrading/internal/customeronboarding/infrastructure"
	"github.com/wyfcoding/financialtrading/internal/customeronboarding/interfaces"
...
func main() {
	// 1. 加载配置
	cfg := &config.Config{}
	if err := config.Load("configs/customeronboarding/config.toml", cfg); err != nil {
	...
	// 5. 自动迁移
	if err := db.DB.AutoMigrate(&domain.OnboardingApplication{}); err != nil {
		logger.Error("failed to migrate database", "error", err)
		os.Exit(1)
	}

	// 6. 依赖注入
	repo := infrastructure.NewGormOnboardingRepository(db.DB)
	app := application.NewOnboardingService(repo)
	handler := interfaces.NewOnboardingHandler(app)
	...
	s := grpc.NewServer()
	pb.RegisterCustomerOnboardingServiceServer(s, handler)
