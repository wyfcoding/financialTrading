...
func main() {
	...
	log.InfoContext(ctx, "Starting PositionService",
	...
	// 10. 创建 HTTP 服务器
	httpServer := createHTTPServer(cfg, positionAppService, rateLimiter)
	...
	log.InfoContext(ctx, "Shutting down PositionService")
	...
	log.InfoContext(ctx, "PositionService stopped")
}

// createHTTPServer 创建 HTTP 服务器
func createHTTPServer(cfg *config.Config, positionAppService *application.PositionApplicationService, rateLimiter ratelimit.RateLimiter) *http.Server {
	router := gin.Default()
	...
	// 注册路由
	httpHandler := httphandler.NewPositionHandler(positionAppService)
...
func createGRPCServer(cfg *config.Config, positionAppService *application.PositionApplicationService) *grpc.Server {
	...
	// 注册服务
	handler := grpchandler.NewGRPCHandler(positionAppService)
