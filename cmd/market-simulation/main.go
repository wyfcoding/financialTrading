...
func main() {
	...
	log.InfoContext(ctx, "Starting MarketSimulationService", "version", cfg.Version)
	...
	<-quit
	log.InfoContext(ctx, "Shutting down MarketSimulationService")
...
func createHTTPServer(cfg *config.Config, app *application.MarketSimulationService, rateLimiter ratelimit.RateLimiter) *http.Server {
	router := gin.Default()
	...
	// 注册路由
	httpHandler := httphandler.NewMarketSimulationHandler(app)
...
func createGRPCServer(cfg *config.Config, app *application.MarketSimulationService) *grpc.Server {
	// 创建 gRPC 服务器选项
	...
	pb.RegisterMarketSimulationServiceServer(server, handler)
