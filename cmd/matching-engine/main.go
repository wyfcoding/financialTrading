...
func main() {
	...
	log.InfoContext(ctx, "Starting MatchingEngineService",
	...
	// 10. 创建 HTTP 服务器
	httpServer := createHTTPServer(cfg, matchingAppService, rateLimiter)
	...
	log.InfoContext(ctx, "Shutting down MatchingEngineService")
	...
	log.InfoContext(ctx, "MatchingEngineService stopped")
}

// createHTTPServer 创建 HTTP 服务器
func createHTTPServer(cfg *config.Config, matchingAppService *application.MatchingApplicationService, rateLimiter ratelimit.RateLimiter) *http.Server {
	router := gin.Default()
