...
func NewExecutionHandler(cmd *application.ExecutionCommandService, query *application.ExecutionQueryService) *ExecutionHandler {
	return &ExecutionHandler{cmd: cmd, query: query}
}
...
func (h *ExecutionHandler) ExecuteOrder(c *gin.Context) {
	...
	dto, err := h.cmd.ExecuteOrder(c.Request.Context(), cmd)
	if err != nil {
...
func (h *ExecutionHandler) SubmitAlgoOrder(c *gin.Context) {
	...
	algoID, err := h.cmd.SubmitAlgoOrder(c.Request.Context(), cmd)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
