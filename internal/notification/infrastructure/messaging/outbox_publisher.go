...
func (OutboxMessage) TableName() string {
	return "notification_outbox_messages"
}
...
func (p *OutboxEventPublisher) PublishNotificationCreated(event domain.NotificationCreatedEvent) error {
	return p.publishEvent("NotificationCreatedEvent", event)
}

// PublishNotificationSent 发布通知发送成功事件
func (p *OutboxEventPublisher) PublishNotificationSent(event domain.NotificationSentEvent) error {
	return p.publishEvent("NotificationSentEvent", event)
}

// PublishNotificationFailed 发布通知发送失败事件
func (p *OutboxEventPublisher) PublishNotificationFailed(event domain.NotificationFailedEvent) error {
	return p.publishEvent("NotificationFailedEvent", event)
}

// PublishNotificationStatusChanged 发布通知状态变更事件
func (p *OutboxEventPublisher) PublishNotificationStatusChanged(event domain.NotificationStatusChangedEvent) error {
	return p.publishEvent("NotificationStatusChangedEvent", event)
}

// PublishBatchNotificationCreated 发布批量通知创建事件
func (p *OutboxEventPublisher) PublishBatchNotificationCreated(event domain.BatchNotificationCreatedEvent) error {
	return p.publishEvent("BatchNotificationCreatedEvent", event)
}
