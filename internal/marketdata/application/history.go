...
// HistoryService 提供基于价格分布的历史分析能力。
// 当前实现为单实例统计（不区分 symbol），用于轻量级实时分析。
type HistoryService struct {
	analyzer domain.HistoryAnalyzer
	maxPrice int
	scale    int
}

func NewHistoryService(analyzer domain.HistoryAnalyzer) *HistoryService {
	return NewHistoryServiceWithConfig(analyzer, defaultHistoryMaxPrice, defaultHistoryPriceScale)
}

func NewHistoryServiceWithConfig(analyzer domain.HistoryAnalyzer, maxPrice, scale int) *HistoryService {
	if maxPrice <= 0 {
